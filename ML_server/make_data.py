...
def main(comment_file_name):
    print("LOAD", "./tmp/"+comment_file_name+".json")
    comments_json = json.load(open('tmp/'+comment_file_name+'.json', "r", encoding="UTF-8"))
    comment_file_name = os.path.splitext(os.path.basename(comment_file_name))[0]
    ...
    data = []
    with open("./tmp/"+comment_file_name+".csv") as f:
